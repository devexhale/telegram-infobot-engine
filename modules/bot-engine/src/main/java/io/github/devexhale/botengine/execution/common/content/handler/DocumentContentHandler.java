...
/**
 * Handles document content by sending document files via the Telegram API.
 *
 * @since 1.0
 */
@Component
...
public class DocumentContentHandler implements ContentHandler {
  ...
  public Message handle(ContentNode node, String chatId) {
    SendDocument request = createRequest(node, chatId);

    try {
      return client.execute(request);
    } catch (TelegramApiException e) {
      log.error("Failed to send document in chat. ChatID={}", chatId);
      throw new TelegramMessageSendException(e);
    }
  }

  private SendDocument createRequest(ContentNode node, String chatId) {
    return SendDocument.builder()
        .chatId(chatId)
        .document(mediaFileLoader.load(node.fileName()))
